...
- 批处理会先检查 `http://127.0.0.1:3080`；若服务已运行，则只打开页面，避免重复启动。
- 发现批处理直接写入中文工作目录时，`cmd.exe` 可能按错误代码页解析并显示乱码；已改为从桌面枚举带有 `10_*` 子目录的工作区来定位，实测启动成功。
...
- 为兼容 Windows 命令行编码，批处理中的工作区定位不再依赖中文目录名文本。

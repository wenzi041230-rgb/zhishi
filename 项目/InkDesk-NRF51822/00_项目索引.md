---
title: InkDesk NRF51822 项目索引
aliases:
  - InkDesk
  - NRF51822墨水屏
tags:
  - project/inkdesk
  - embedded/nrf51822
  - display/eink
status: active
updated: 2026-08-15
---

# InkDesk NRF51822 项目索引

InkDesk 是一块由手机编辑、通过 BLE 发送、在 400×300 黑白红墨水屏上长期显示的低打扰桌面信息牌。

## 当前结论

- 硬件基础：NRF51822-QFAC-R、S130、SSD1683、400×300 三色墨水屏、外置 8 Mbit Flash。
- 产品固件：v1.0.0 已完成编译和打包，应用版本号为 6，应用大小 24,524 字节。
- 交接固件：原始 `epd_ble_xfer` 与 InkDesk v1.0.0 不是同一代码线；交接版应用大小 20,848 字节，使用外部 X2 32.768 kHz LFCLK。
- 已实现：BLE NUS、双平面传图、清屏、时钟、时间同步、设备信息、运行状态、BUSY 保护、自检和 Secure DFU。
- 已验证：源码干净构建、内存布局、DFU 包签名元数据、Bootloader settings CRC；2026-08-15 已通过 J-Link App-only 烧录并完成应用区回读哈希一致性验证。
- 历史实板证据：原始 `epd_ble_xfer` 固件曾保持 BLE 连接 2 小时，710 次 PING 全部成功、0 次重连；这证明该板的 BLE 链路至少在当时可用。
- 运行证据：产品版和交接版均能从 Bootloader 启动，J-Link 采样均见 RADIO 在三个 BLE 广播信道活动，HFCLK 活动期使用外部 16 MHz 晶振，设备未连接其他主机。
- 当前现场状态：交接版已实际替换到板上并完成回读哈希一致性验证，但当前 Windows 主机仍未扫描到 `EPD_Ink`；交接版 RADIO 内部活动已证实，天线后的空口可见性和后续协议链路尚未闭环。
- 2026-08-15 再次执行 v1.0.0 App-only 流程，J-Link 判定板上应用/settings 已完全一致；复位和回读通过，但 BLE 扫描现象未改变。
- 尚待实板回归：广播可见性、信息查询、时间同步、三色测试图、完整传图与 OTA。

## 系统结构

```mermaid
flowchart LR
    A["Android App\n编辑、天气、图片处理"] -->|"BLE NUS"| B["NRF51822 产品固件"]
    B --> C["SSD1683 控制器"]
    C --> D["400×300 黑白红墨水屏"]
    B --> E["Secure DFU Bootloader"]
    E -->|"DfuTarg"| A
    B -.-> F["外置 8 Mbit Flash\n后续多页缓存"]
```

## 知识导航

- [[01_产品定义与范围]]
- [[02_硬件架构与引脚]]
- [[03_固件架构与内存布局]]
- [[04_BLE产品协议]]
- [[05_墨水屏帧缓冲与刷新]]
- [[06_时钟系统]]
- [[07_构建烧录与DFU]]
- [[08_故障排查与最短验证路径]]
- [[09_产品与技术路线图]]
- [[10_交付物与安全边界]]

## 下一次接手时先做

1. 确认屏幕刷新端有 5 V，逻辑电源约为 3.3 V。
2. 用 J-Link 只升级 App 和匹配的 settings，避免无必要的整片擦除。
3. 扫描 `EPD_Ink`，依次执行 `PING → INFO → CAPS → STATUS`。
4. 同步时间，等待 `TIME_OK` 和 `CLK_OK`。
5. 发送 `TEST bands`，确认白、黑、红三段正常。
6. 发送完整 BW/RED 图像，只有收到 `SHOW_OK` 才判定成功。
